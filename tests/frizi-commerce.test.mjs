...
test('catalogue separates purchasable Canadian-approved products from blocked products', () => {
  const catalogue = getCommerceCatalogue('preview_client');
  ...
  assert.equal(approved.recommendation.id, 'rec_preview_leavein');
...
test('calculates product cart, promotion, shipping, Ontario tax, and commission attribution', () => {
  const summary = calculateCommerceCart({
    ...
    items: [{ variantId: 'var_pattern_leave_in_289ml', quantity: 1, recommendationId: 'rec_preview_leavein' }],
  ...
  assert.equal(summary.items[0].professionalName, 'Preview Professional 1');
...
test('free shipping applies automatically above configured threshold', () => {
  const summary = calculateCommerceCart({
    ...
    items: [
      ...
      { variantId: 'var_briogeo_mask_240ml', quantity: 1, recommendationId: 'rec_preview_mask' },
    ],
